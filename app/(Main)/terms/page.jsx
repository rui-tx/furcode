import React from "react";
import "./styles/index.css";

const Section = ({ title, children }) => (
  <div className="policy-section">
    <h2>{title}</h2>
    {children}
  </div>
);

const TermsAndConditionsPage = () => {
  return (
    <div className="terms-policy-container">
      <h1>Termos e Condições</h1>

      <Section title="">
        <p>
          Estes Termos e Condições destinam-se a regulamentar o uso do site da
          PetHub.
        </p>
        <br />
        <p>
          Qualquer informação a que o utilizador do site tenha acesso não deve
          ser utilizada para fins ilegais ou não autorizados. Este website é
          propriedade da PetHub e gerido por ela. Os Termos e Condições são
          interpretados de acordo com a lei da União Europeia.
        </p>
        <br />
        <p>
          O utilizador concorda em não atribuir ou fazer qualquer uso comercial
          do conteúdo ou serviços do website sem o consentimento expresso da
          PetHub.
        </p>
        <br />
        <p>
          A navegação no site da PetHub pressupõe a leitura e aceitação das
          condições apresentadas nestes Termos e Condições.
        </p>
        <br />
        <p>
          Os Termos e Condições apresentados podem ser alterados, adicionados ou
          eliminados a qualquer momento. Recomenda-se que consulte estes Termos
          e Condições periodicamente para verificar eventuais atualizações.
        </p>
        <br />
        <p>
          Os Termos e Condições estão publicados no site da PetHub para garantir
          os princípios de informação, comunicação e transparência.
        </p>
        <br />
        <p>
          Quem aceder a este website, seja um indivíduo particular ou um
          representante de uma organização, navegar nas páginas ou aceitar estes
          Termos e Condições, é chamado de utilizador do website. O website está
          alojado no endereço www.PetHub.com.
        </p>
      </Section>

      <Section title="Ao utilizar este website são proibidos:">
        <p>
          <strong>a.</strong> Quaisquer atividades inadequadas e/ou ilegais.
        </p>
        <br />
        <p>
          <strong>b.</strong> Violação de qualquer norma legal, regulatória ou
          outras normas nacionais ou internacionais.
        </p>
        <br />
        <p>
          <strong>c.</strong> Violação da privacidade (incluindo o envio de
          informações privadas para o website sem o consentimento do titular dos
          dados pessoais) ou de quaisquer outros direitos legais dos titulares
          dos dados pessoais.
        </p>
        <br />
        <p>
          <strong>d.</strong> Difamação ou tentativa de prejudicar a reputação,
          imagem, credibilidade, respeitabilidade e bom nome de qualquer titular
          de dados pessoais ou da PetHub.
        </p>
        <br />
        <p>
          <strong>e.</strong> Uso ou transferência de ficheiros que contenham
          agentes maliciosos capazes de causar danos ao website e, direta ou
          indiretamente, à propriedade de titulares de dados pessoais ou que
          permitam o acesso a qualquer rede, servidor ou sistema informático
          ligado ao website.
        </p>
        <br />
        <p>
          <strong>f.</strong> Transferência de conteúdo não autorizado, em
          qualquer formato, incluindo conteúdo ilegal, difamatório ou que cause
          danos ou perda, ou que viole os sistemas de segurança da rede ou
          segurança de terceiros.
        </p>
        <br />
        <p>
          <strong>g.</strong> Alterações ao website, a menos que autorizadas
          pela PetHub.
        </p>
        <br />
        <p>
          <strong>h.</strong> Violação da legislação sobre direitos de autor,
          imagem, marca ou direitos de propriedade intelectual, da PetHub, de
          quaisquer terceiros ou de quaisquer titulares de dados pessoais.
        </p>
        <br />
        <p>
          Não é possível prevenir situações de ataque ou a invulnerabilidade do
          website a ataques, vírus ou outro software/código malicioso ou
          intrusivo. Nesse sentido, a PetHub não se responsabiliza por quaisquer
          danos que possam resultar para o utilizador em virtude de violações
          causadas por terceiros.
        </p>
        <br />
        <p>
          Os direitos de Proteção de Dados podem ser consultados na nossa
          política de privacidade.
        </p>
        <br />
      </Section>

      <Section title="Limitação de responsabilidade:">
        <p>
          <strong>a.</strong> O utilizador concorda expressamente em assumir
          exclusivamente todos e quaisquer riscos decorrentes do uso do website
          ou do compartilhamento de dados pessoais referidos anteriormente de
          forma ilegal ou sem autorização do titular.
        </p>
        <br />
        <p>
          <strong>b.</strong> A PetHub não se responsabiliza pela incapacidade
          de uso, interrupção de negócios, ou por danos diretos ou indiretos,
          especiais, acidentais ou consequenciais de qualquer tipo por
          negligência.
        </p>
        <br />
        <p>
          <strong>c.</strong> A PetHub não garante que o website atende aos
          requisitos do utilizador, que o website seja ininterrupto, ou que seja
          seguro em redes abertas, oportuno ou livre de erros, nem garante os
          resultados obtidos com o uso do website ou a precisão ou
          confiabilidade das informações obtidas através do website. Da mesma
          forma, não garante a correção de defeitos no website.
        </p>
        <br />
        <p>
          <strong>Exatidão e integridade da informação</strong>
        </p>
        <br />
        <p>
          <strong>a.</strong> Procuramos assegurar, em todos os momentos, que a
          informação disponibilizada no website é correta, adequada e
          devidamente contextualizada.
        </p>
        <p>
          <strong>Interrupção ou suspensão de acesso</strong>
        </p>
        <br />
        <p>
          <strong>a.</strong> Salvaguardamos o direito de interromper o acesso
          ao website por qualquer período necessário, seja por razões técnicas,
          legais, organizacionais, administrativas, ou por força maior, sem
          excluir outras razões que possam ocorrer e não estejam previstas
          nestes Termos e Condições.
        </p>
        <p>
          <strong>Direitos de Propriedade Intelectual</strong>
        </p>
        <br />
        <p>
          <strong>a.</strong> Todo o conteúdo deste website, como fotografias,
          desenhos, gráficos, ícones, textos, software, nomes comerciais,
          marcas, logótipos ou quaisquer outros sinais distintivos suscetíveis
          de uso industrial e comercial, são propriedade da PetHub ou de
          terceiros que devidamente autorizaram a sua inclusão neste website e
          estão protegidos por Direitos de Autor e Direitos Conexos, e Direitos
          de Propriedade Industrial, de acordo com as leis da União Europeia,
          convenções internacionais e outras leis.
        </p>
        <br />
        <p>
          <strong>b.</strong> O utilizador não pode reproduzir, modificar,
          transformar, desmontar, realizar engenharia reversa, distribuir,
          alugar, fornecer, disponibilizar ou permitir o acesso ao público,
          através de qualquer forma de comunicação pública, de quaisquer dos
          elementos referidos no parágrafo anterior (Direitos de Propriedade
          Intelectual, seção a). O utilizador deve usar os materiais, elementos
          e informações que acede através deste website apenas para as suas
          necessidades pessoais, sendo expressamente proibida a exploração
          comercial direta ou indireta dos mesmos, exceto com a autorização
          prévia e por escrito da PetHub ou, dependendo dos casos, dos titulares
          de direitos de propriedade intelectual e industrial de terceiros.
        </p>
      </Section>

      <Section title="Disposições finais e contactos">
        <p>
          Para qualquer informação relacionada com os Termos e Condições de uso
          do website da PetHub, podem ser utilizados os seguintes contactos:
        </p>
        <br />
        <p>PetHub@minderacodeacademy.com</p>
      </Section>
    </div>
  );
};

export default TermsAndConditionsPage;
